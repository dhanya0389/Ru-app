...
export default function ThemePicker() {
  ...
  return (
    <div ref={menuRef} className="fixed bottom-6 right-6 z-50">
      {/* Expanded menu */}
      {open && (
        <div className="absolute bottom-14 right-0 bg-white/80 backdrop-blur-md rounded-2xl p-4 shadow-xl
                        border border-white/50 screen-enter" style={{ minWidth: '180px' }}>
          <p className="text-xs text-ruhi-earth/50 mb-3 font-medium">Pick your vibe</p>
          <div className="flex flex-col gap-2">
            {Object.entries(THEMES).map(([key, theme]) => (
              ...
              >
                ...
                {active === key && (
                  <span className="text-xs text-ruhi-earth/50 ml-auto">✓</span>
                )}
              </button>
            ))}
          </div>
